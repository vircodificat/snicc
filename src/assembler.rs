...
impl Assembler {
    ...
    fn assemble_func(&mut self, func: &tac::Func) -> asm::Func {
        ...
        self.assemble_func_alloca(func, &mut instrs);
    ...
    fn assemble_func_alloca(&mut self, func: &tac::Func, instrs: &mut Vec<asm::Instr>) {
        ...
        for instr in &block.instrs {
            match instr {
                tac::Instr::Ret(ssa) => {
                    ...
                    let total_size: usize = self.ssa_stack_offsets.values().sum();
